...
@cli.command()
def scribe(file: Path, model_size: str = "small"):
    """Transcribe an audio file using the Faster-Whisper model."""

    model = WhisperModel(model_size)
    segments, info = model.transcribe(file.as_posix(), beam_size=5)
...
def serve(host: str = "127.0.0.1", port: int = 8000, model_size: str = "small"):
    ...
    app.extra["model"] = WhisperModel(model_size)
